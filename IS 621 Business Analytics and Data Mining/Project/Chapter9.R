...
setwd("//Users//Charley//Downloads//cuny//IS 621 Business Analytics and Data Mining//Project")
...
g$residuals
testdata <- data.frame(x=as.numeric(tail(g$residuals,-1)), 
                       y=as.numeric(head(g$residuals,-1)))
cor(tail(testdata$x,-1), tail(testdata$y,-1))

cor(residuals(m1)[-1], residuals(m1)[-length(residuals(g))])

acf(m1$residuals)$acf[2]
